...
from code.Const import C_WHITE, WIN_HEIGHT, MENU_OPTION, EVENT_ENEMY, \
    SPAWN_TIME, C_GREEN, C_CYAN, EVENT_TIMEOUT, TIMEOUT_STEP, TIMEOUT_LEVEL
...
class Level:
    def __init__(self, window: Surface, name: str,
                 game_mode: str, player_score: list[int]):
    ...
    def run(self, player_score: list[int]):
        ...
        if self.name == 'Level3':  # time out set for level 3
            self.timeout *= 2
        ...
        while True:
            ...
            for ent in self.entity_list:
                ...
                if ent.name == 'Player1':
                    self.level_text(14, f'Player1 - Health: {ent.health} |\
                                     Score: {ent.score}', C_GREEN, (10, 25))
                if ent.name == 'Player2':
                    self.level_text(14, f'Player2 - Health: {ent.health} |\
                                     Score: {ent.score}', C_CYAN, (10, 45))
            for event in pygame.event.get():
                ...
                if event.type == EVENT_ENEMY:
                    ...
                    elif self.name == 'Level3':  # only enemy3 on level 3
                        self.entity_list.append(
                            EntityFactory.get_entity('Enemy3'))
                if event.type == EVENT_TIMEOUT:
                    self.timeout -= TIMEOUT_STEP
                    if self.timeout == 0:
                        for ent in self.entity_list:
                            if isinstance(ent, Player) and\
                                    ent.name == 'Player1':
